...
// Endpoint to fetch and save data (if needed)
...
function toFloatIfValid(value) {
  ...
  return isNaN(number) ? { minPrice: null, maxPrice: null } : { minPrice: number - 100, maxPrice: number + 100 };
}
...
app.get('/transactions', async (req, res) => {
  ...
  try {
    ...
    res.json({ products: paginatedResults, total, page: parseInt(page), perPage: parseInt(perPage) });
  } catch (error) {
